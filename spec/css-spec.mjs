...
describe('CSS grammar', function () {
	...
	describe('CSS at-rule coverage', function () {
		...
		it('recovers from an unclosed parenthesis in a @container or @scope prelude', function () {
			// The parenthesised regions used to end only on `)`, so an unclosed
			// prelude leaked its header scope to the end of the document. The
			// bail-out fires on a `{` that nothing closes later on the line. A
			// `;` is deliberately *not* a bail-out: `<any-value>` permits a
			// top-level one, so treating it as an error breaks legal syntax.
			[
				['@container (width > 1px{', ['source.css', 'meta.at-rule.container.body.css']],
				['@container style(--theme: dark{', ['source.css', 'meta.at-rule.container.body.css']],
				['@container scroll-state(stuck: top{', ['source.css', 'meta.at-rule.container.body.css']],
				['@scope (:is(.a){', ['source.css', 'meta.at-rule.scope.body.css']],
				['@container (width > 1px', ['source.css']],
				['@container scroll-state(stuck: top', ['source.css']],
		...
		it('recovers a style() query that is cut off by an unclosed brace', function () {
			// `{` ends the query only when it ends the line, so the brace that
			// follows a truncated query opens the container body rather than
			// being absorbed into the value.
			var lines = testGrammar.tokenizeLines('@container style(--theme: dark{\n.after { color: red; }\n}');
			assert.deepStrictEqual(lines[0].find(t => t.value === '{').scopes, ['source.css', 'meta.at-rule.container.body.css', 'punctuation.section.container.begin.bracket.curly.css']);
			assert.deepStrictEqual(lines[1].find(t => t.value === 'after').scopes, ['source.css', 'meta.at-rule.container.body.css', 'meta.selector.css', 'entity.other.attribute-name.class.css']);
		});
		...
		it('keeps a brace that the query itself closes, in every guarded region', function () {
			// One case per region carrying the `[^)]*$` guard, so that dropping
			// the guard from any one of them fails a test. A `{` closed later
			// on the same line belongs to the query, not to the rule that
			// follows it.
			[
				['@scope (.a) to (.b { c })', 'scope'],
				['@scope (.a { b })', 'scope'],
		...
		it('recovers every scroll-state() condition that is cut off by a brace', function () {
			// `stuck`, `snapped` and `scrollable` each have their own condition
			// region, so each needs its own bail-out.
			['stuck: top', 'snapped: x', 'scrollable: x'].forEach(function (condition) {
				...
				assert.deepStrictEqual(lines[0].find(t => t.value === '{').scopes, ['source.css', 'meta.at-rule.container.body.css', 'punctuation.section.container.begin.bracket.curly.css'], condition);
				assert.deepStrictEqual(lines[1].find(t => t.value === 'after').scopes, ['source.css', 'meta.at-rule.container.body.css', 'meta.selector.css', 'entity.other.attribute-name.class.css'], condition);
			});
		});
		...
		it('recovers when the unclosed parenthesis is a nested one', function () {
			// The guard on an outer prelude region cannot fire while an inner
			...
			[
				...
				'@container (width > calc(10px + 2em{',
				'@scope (:is(.a{',
				'@scope (.a) to (:where(.b{',
				'@scope (:nth-child(2 of .special{',
				'@container (width > 10px) and (height < calc(2em{'
			].forEach(function (prelude) {
				var lines = testGrammar.tokenizeLines(prelude + '\n.after { color: red; }');
		...
		it('recovers for every guarded region reachable inside a prelude', function () {
			// Mutating each guard site individually showed which sites the
			// suite actually pins; these cover the rest. Of the 36 sites, 35
			// are caught by a failing test, and the one survivor,
			// `@document`'s argument functions, recovers no better with the
			// guard than without it, exactly as on main.
			// Value functions are reached through `@container style()`,
			// selectors through `@scope`.
			[
				'@container style(--x: layer(a{',
				'@container style(--x: rgb(255{',
				'@container style(--x: oklch(50%{',
				'@container style(--x: linear-gradient(red{',
				'@container style(--x: -webkit-gradient(linear{',
				'@container style(--x: polygon(0 0{',
				'@container style(--x: cubic-bezier(0,0{',
				'@container style(--x: steps(2{',
				'@container style(--x: anchor(top{',
				'@container style(--x: anchor-size(width{',
				'@container style(--x: clamp(1px{',
				'@container style(--x: translate(1px{',
				'@scope (:dir(ltr{',
				'@scope (:state(foo{',
				'@scope (::part(box{',
				'@scope (::highlight(h{',
		...
		it('recovers when a closing parenthesis is shielded by a comment, string or escape', function () {
			// The bail-out asks whether the rest of the line closes the
			// parenthesis. A `)` inside a comment, string or escape does not,
			// so it must not suppress recovery. `origin/main` recovers the
			// first three of these because its generic header ends at `{`.
			[
				'@container (width > 1px{ /* ) */',
				'@media (min-width: 40em{ /* note (x) */',
				'@container (width > 1px{ \\)',
				'@container (width > 1px{ "x)"',
				"@container (width > 1px{ 'x)'",
				'@supports (a{ /* ) */',
				// The scroll-state features and the `of` clause reach the brace
				// through a lookahead-only `end`, so they need the same bail-out.
				'@container scroll-state(stuck: top{ /* ) */',
				'@container scroll-state(snapped: x{ "x)"',
		...
		it('recovers when the prelude line ends in a line-continuation backslash', function () {
			...
			[
				...
				// The string alternatives accepted only complete escapes, so
				// an odd number of trailing backslashes matched nothing.
				'@scope (.a{ "x\\',
				'@scope (.a{ \'x\\',
				'@container (width > 1px{ "x\\\\\\',
				'@container (width > 1px{ "x\\'
		...
		it('does not bail out when the line really does close the parenthesis', function () {
			// The mirror of the case above: a genuine `)` after the brace means
			// the prelude is still open, and recovery must not fire.
		...
		it('still opens the body when a prelude parenthesis is left unclosed before `{`', function () {
			[
				...
				['@container scroll-state(stuck: top{', 'container'],
				['@scope (.a {', 'scope']
			].forEach(function (probe) {
				var lines = testGrammar.tokenizeLines(probe[0] + '\n  color: red;\n}');
		...
		it('keeps scoping the closing parenthesis of a well-formed prelude', function () {
			...
			tokens = testGrammar.tokenizeLine('@scope (.a) to (.b) {').tokens;
			assert.deepStrictEqual(tokens.find(t => t.value === ')').scopes, ['source.css', 'meta.at-rule.scope.header.css', 'meta.scope.limit.css', 'punctuation.definition.parameters.end.bracket.round.css']);
		});
	});

	describe('CSS function coverage', function () {
		...
		it('bounds an unterminated env() to a single argument name', function () {
			...
			// region itself releases at a `{` that nothing closes later on the
			// line, so a following rule is not swallowed.
			...
			assert.ok(!lines[1].find(t => t.value === 'red').scopes.includes('meta.function.env.css'));
		});
		...
		it('keeps functions that the css-values-5 draft still defines', function () {
			['progress', 'toggle'].forEach(function (fn) {
	...
	describe('CSS property coverage', function () {
		...
		it('recognises property names promoted out of the deprecated list', function () {
	...
	describe('CSS selector coverage', function () {
	...
	describe('CSS media feature coverage', function () {
